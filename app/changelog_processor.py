...
from flask_service_tools import AIGatewayClient, \
    Config  # Gardez Config si utilisé ailleurs, sinon il peut être retiré des imports ici
...
class ChangelogProcessor:
    ...
    def _process_single_changelog_line(self, line_row: dict) -> str:
        ...
        if pr_identification_result['status'] == 'success':
            ...
            print(f"   PR #{pr_number_identified} IDENTIFIÉE ({pr_info.get('method', 'N/A')}). DIFF 🔄 Récupération...")
    ...
    def process_changelog_lines_refactored(self,
        ...
        for line_row in lines_to_process:
            ...
            if log_entry:  # S'assurer qu'on a bien un log à ajouter
    ...
    def _attempt_pr_identification(self, line_content: str):
        ...
        if pr_number_from_text:
            print(f"  PR #️⃣ Numéro PR {pr_number_from_text} extrait du texte.")
    ...
    def determine_line_type_and_process_db(self, section_lines: list[str]):  # db_handler est maintenant self.db_handler
        ...
        """
        print(f"ℹ️ Préparation de l'insertion des lignes de contenu dans la table {self.db_handler.table_name}...")
        ...
        for line_text in section_lines:
            ...
            if stripped_line_lower in user_section_triggers:
                ...
                print(f" Contexte changé à : {current_db_line_type} (section: {stripped_line_lower})")
                continue
    ...
    def get_pr_details_by_number(self, pr_number: int):
        ...
        """
        print(f"  PR INFO ↔️ Tentative de récupération des détails pour PR #{pr_number}")
